...
int MovieDecoder::getFps()
{
    if (m_pVideoStream->avg_frame_rate.den != 0) {
        return av_q2d(m_pVideoStream->avg_frame_rate);
    } else if (m_pVideoStream->r_frame_rate.den != 0) {
...
void MovieDecoder::initializeFilterGraph(const AVRational& timeBase, const std::string& size, bool maintainAspectRatio)
{
    ...
    checkRc(avfilter_graph_create_filter(&formatFilter, avfilter_get_by_name("format"), "thumb_format", "pix_fmts=bgr24", nullptr, m_pFilterGraph),
...
void MovieDecoder::GetAspectRatio(int& ascpectNominator, int& ascpectDenominator)
{
    if(m_pVideoCodecContext != nullptr) {
        if (m_pVideoCodecContext->sample_aspect_ratio.num != 0 && m_pVideoCodecContext->sample_aspect_ratio.den != 0) {
            ascpectNominator = m_pVideoCodecContext->sample_aspect_ratio.num;
            ascpectDenominator = m_pVideoCodecContext->sample_aspect_ratio.den;
            return;
        }
	}
}
...
cv::Mat MovieDecoder::getScaledVideoFrame(const std::string& scaledSize, bool maintainAspectRatio)
{
    if(isNextEmpty)
		return cv::Mat();
	...
	videoFrame = cv::Mat(res->height, res->width, CV_8UC3);
	memcpy(videoFrame.data, res->data[0], videoFrame.total() * videoFrame.elemSize());

	//Decode next frame in case the current one is not key frame, otherwise the next seek will fail
    try
    {
        decodeVideoFrame();
    }
