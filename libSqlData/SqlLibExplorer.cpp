...
#define SQL_CREATE_VIEW_PHOTOSWITHOUTTHUMBNAIL "CREATE VIEW PHOTOSWIHOUTTHUMBNAIL_VIEW AS \
            SELECT FullPath From PHOTOS WHERE FullPath not in (SELECT FullPath From PHOTOSTHUMBNAIL) or FullPath in (select FullPath from PHOTOS where MultiFiles = 1)  GROUP BY fullpath HAVING COUNT(*) < 20"
