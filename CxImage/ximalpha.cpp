...
bool CxImage::AlphaCreate()
{
	if (pAlpha == nullptr)
	{
		pAlpha = static_cast<uint8_t*>(malloc(head.biWidth * head.biHeight));
	...
	if (head.biBitCount == 24)
	{
		for (int32_t y = 0; y < head.biHeight; y++)
		{
			for (int32_t x = 0; x < head.biWidth; x++)
			{
				...
				a1 = 256 - a;
				c.rgbBlue = static_cast<uint8_t>((c.rgbBlue * a + a1 * info.nBkgndColor.rgbBlue) >> 8);
				c.rgbGreen = static_cast<uint8_t>((c.rgbGreen * a + a1 * info.nBkgndColor.rgbGreen) >> 8);
				c.rgbRed = static_cast<uint8_t>((c.rgbRed * a + a1 * info.nBkgndColor.rgbRed) >> 8);
...
bool CxImage::AlphaFlip()
{
	...
	auto buff = static_cast<uint8_t*>(malloc(head.biWidth));
...
{
	if (!pAlpha) return false;
	auto pAlpha2 = static_cast<uint8_t*>(malloc(head.biWidth * head.biHeight));
