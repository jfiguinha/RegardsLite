...
void CxImage::Copy(const CxImage& src, bool copypixels, bool copyselection, bool copyalpha)
{
	...
	if (copyselection && src.pSelection)
	{
		...
		pSelection = static_cast<uint8_t*>(malloc(nSize));
	...
	if (copyalpha && src.pAlpha)
	{
		...
		pAlpha = static_cast<uint8_t*>(malloc(nSize));
...
void* CxImage::Create(uint32_t dwWidth, uint32_t dwHeight, uint32_t wBpp, uint32_t imagetype)
{
	...
	if (((static_cast<float>(dwWidth) * static_cast<float>(dwHeight) * static_cast<float>(wBpp)) / 8.0f) > static_cast<
		float>(CXIMAGE_MAX_MEMORY))
	{
	...
	lpbi = static_cast<BITMAPINFOHEADER*>(pDib);
	*lpbi = head;
...
uint8_t* CxImage::GetBits(uint32_t row)
{
	if (pDib)
	{
		if (row)
		{
			...
			{
				return (static_cast<uint8_t*>(pDib) + *static_cast<uint32_t*>(pDib) + GetPaletteSize() + (info.
					dwEffWidth * row));
			}
			return nullptr;
		}
		return (static_cast<uint8_t*>(pDib) + *static_cast<uint32_t*>(pDib) + GetPaletteSize());
	}
	return nullptr;
}
 ...
 * \return true if everything is ok
 */
bool CxImage::CreateFromArray(uint8_t* pArray, int size, uint32_t dwWidth, uint32_t dwHeight, uint32_t dwBitsperpixel,
                              uint32_t dwBytesperline, bool bFlipImage, bool rgb)
{
	...
	if (!((dwBitsperpixel == 1) || (dwBitsperpixel == 4) || (dwBitsperpixel == 8) ||
		(dwBitsperpixel == 24) || (dwBitsperpixel == 32)))
		return false;
	...
	for (uint32_t y = 0; y < dwHeight; y++)
	{
		...
		{
			memcpy(dst, src, min((size_t)info.dwEffWidth, (size_t)dwBytesperline));
		}
	}
 ...
 * \sa CreateFromArray
 */
bool CxImage::CreateFromMatrix(uint8_t** ppMatrix, uint32_t dwWidth, uint32_t dwHeight, uint32_t dwBitsperpixel,
...
{
	auto c1 = (RGBQUAD*)elem1;
	auto c2 = (RGBQUAD*)elem2;

	int32_t g1 = RGB2GRAY(c1->rgbRed, c1->rgbGreen, c1->rgbBlue);
	int32_t g2 = RGB2GRAY(c2->rgbRed, c2->rgbGreen, c2->rgbBlue);
