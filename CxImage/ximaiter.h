...
{
	if (++Ity >= static_cast<int32_t>(ima->GetHeight())) return 0;
...
{
	if ((y < 0) || (y > static_cast<int32_t>(ima->GetHeight()))) return;
	Ity = y;
...
{
	if (n < 0) n = static_cast<int32_t>(ima->GetEffWidth());
...
{
	if (++Itx < static_cast<int32_t>(ima->GetEffWidth())) return 1;
