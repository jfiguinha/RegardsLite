...
size_t CxMemFile::Write(const void* buffer, size_t size, size_t count)
{
	...
	if (m_Position > static_cast<int32_t>(m_Size)) m_Size = m_Position;
...
{
	if (m_pBuffer == nullptr || m_Position >= static_cast<int32_t>(m_Size))
	{
		m_bEOF = true;
		return EOF;
	}
...
bool CxMemFile::Alloc(uint32_t dwNewLen)
{
	if (dwNewLen > static_cast<uint32_t>(m_Edge))
	{
		...
		uint32_t dwNewBufferSize = (dwNewLen >> 16) + 1 << 16;

		// allocate new buffer
		if (m_pBuffer == nullptr) m_pBuffer = static_cast<uint8_t*>(malloc(dwNewBufferSize));
		else m_pBuffer = static_cast<uint8_t*>(realloc(m_pBuffer, dwNewBufferSize));
