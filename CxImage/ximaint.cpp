 ...
 *  \author ***bd*** 2.2004
 */
void CxImage::OverflowCoordinates(int32_t& x, int32_t& y, const OverflowMethod ofMethod)
{
	...
	case OM_REPEAT:
		//clip coordinates
		x = max(x, 0);
		x = min((int32_t)x, (int32_t)head.biWidth-1);
		y = max(y, 0);
		y = min((int32_t)y, (int32_t)head.biHeight-1);
		break;
	case OM_WRAP:
		//wrap coordinates
		x = x % head.biWidth;
 ...
 * \author ***bd*** 2.2004
 */
void CxImage::OverflowCoordinates(float& x, float& y, const OverflowMethod ofMethod)
{
	...
	case OM_REPEAT:
		//clip coordinates
		x = max((float)x, (float)0);
		x = min((float)x, (float)head.biWidth-1);
		y = max((float)y, (float)0);
		y = min((float)y, (float)head.biHeight-1);
		break;
	case OM_WRAP:
		//wrap coordinates
 ...
 * \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetPixelColorWithOverflow(int32_t x, int32_t y, const OverflowMethod ofMethod, RGBQUAD* const rplColor)
{
	RGBQUAD color; //color to return
	if ((!IsInside(x, y)) || pDib == nullptr)
	{
		...
		case OM_BACKGROUND:
			...
			{
				if (head.biBitCount < 24) color = GetPaletteColor(static_cast<uint8_t>(info.nBkgndIndex));
	...
	RGBQUAD* const rplColor)
{
	...
	case IM_NEAREST_NEIGHBOUR:
		return GetPixelColorWithOverflow(static_cast<int32_t>(x + 0.5f), static_cast<int32_t>(y + 0.5f), ofMethod,
		                                 rplColor);
	default:
		{
			...
			if ((xi + 1) < head.biWidth && xi >= 0 && (yi + 1) < head.biHeight && yi >= 0 && head.biClrUsed == 0)
			{
				...
				uint16_t wt1 = static_cast<uint16_t>((x - xi) * 256.0f), wt2 = static_cast<uint16_t>((y - yi) * 256.0f);
				uint16_t wd = wt1 * wt2 >> 8;
				uint16_t wb = wt1 - wd;
				uint16_t wc = wt2 - wd;
				uint16_t wa = 256 - wt1 - wc;
				uint16_t wrr, wgg, wbb;
				uint8_t* pxptr = info.pImage + yi * info.dwEffWidth + xi * 3;
				wbb = wa * (*pxptr++);
				wgg = wa * (*pxptr++);
				wrr = wa * (*pxptr++);
				wbb += wb * (*pxptr++);
			...
			color.rgbRed = static_cast<uint8_t>(a * rgb11.rgbRed + b * rgb21.rgbRed + c * rgb12.rgbRed + d * rgb22.
				rgbRed);
			color.rgbGreen = static_cast<uint8_t>(a * rgb11.rgbGreen + b * rgb21.rgbGreen + c * rgb12.rgbGreen + d *
				rgb22.rgbGreen);
			color.rgbBlue = static_cast<uint8_t>(a * rgb11.rgbBlue + b * rgb21.rgbBlue + c * rgb12.rgbBlue + d * rgb22.
				rgbBlue);
#if CXIMAGE_SUPPORT_ALPHA
			color.rgbReserved = static_cast<uint8_t>(a * rgb11.rgbReserved + b * rgb21.rgbReserved + c * rgb12.
				rgbReserved + d * rgb22.rgbReserved);
#else
		color.rgbReserved = 0;
#endif
			return color;
	...
	//calculate multiplication factors for all pixels
		...
		case IM_BICUBIC:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelCubic(xi + i - 1 - x);
				kernely[i] = KernelCubic(yi + i - 1 - y);
			} //for i
			break;
		case IM_BICUBIC2:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelGeneralizedCubic(xi + i - 1 - x, -0.5);
				kernely[i] = KernelGeneralizedCubic(yi + i - 1 - y, -0.5);
			} //for i
			break;
		case IM_BSPLINE:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelBSpline(xi + i - 1 - x);
				kernely[i] = KernelBSpline(yi + i - 1 - y);
			} //for i
			break;
		case IM_BOX:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelBox(xi + i - 1 - x);
				kernely[i] = KernelBox(yi + i - 1 - y);
			} //for i
			break;
		case IM_HERMITE:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelHermite(xi + i - 1 - x);
				kernely[i] = KernelHermite(yi + i - 1 - y);
			} //for i
			break;
		case IM_HAMMING:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelHamming(xi + i - 1 - x);
				kernely[i] = KernelHamming(yi + i - 1 - y);
			} //for i
			break;
		case IM_SINC:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelSinc(xi + i - 1 - x);
				kernely[i] = KernelSinc(yi + i - 1 - y);
			} //for i
			break;
		case IM_BLACKMAN:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelBlackman(xi + i - 1 - x);
				kernely[i] = KernelBlackman(yi + i - 1 - y);
			} //for i
			break;
		case IM_BESSEL:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelBessel(xi + i - 1 - x);
				kernely[i] = KernelBessel(yi + i - 1 - y);
			} //for i
			break;
		case IM_GAUSSIAN:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelGaussian(xi + i - 1 - x);
				kernely[i] = KernelGaussian(yi + i - 1 - y);
			} //for i
			break;
		case IM_QUADRATIC:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelQuadratic(xi + i - 1 - x);
				kernely[i] = KernelQuadratic(yi + i - 1 - y);
			} //for i
			break;
		case IM_MITCHELL:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelMitchell(xi + i - 1 - x);
				kernely[i] = KernelMitchell(yi + i - 1 - y);
			} //for i
			break;
		case IM_CATROM:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelCatrom(xi + i - 1 - x);
				kernely[i] = KernelCatrom(yi + i - 1 - y);
			} //for i
			break;
		case IM_HANNING:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelHanning(xi + i - 1 - x);
				kernely[i] = KernelHanning(yi + i - 1 - y);
			} //for i
			break;
		case IM_POWER:
			for (i = 0; i < 4; i++)
			{
				kernelx[i] = KernelPower(xi + i - 1 - x);
				kernely[i] = KernelPower(yi + i - 1 - y);
			} //for i
			break;
		} //switch
		rr = gg = bb = aa = 0;
		if (((xi + 2) < head.biWidth) && xi >= 1 && ((yi + 2) < head.biHeight) && (yi >= 1) && !IsIndexed())
		{
			...
			{
				auto pxptr = static_cast<uint8_t*>(BlindGetPixelPointer(xi - 1, yii));
	...
	case IM_LANCZOS:
		...
		for (xii = xi - 5; xii < xi + 7; xii++) kernelx[xii - (xi - 5)] = KernelLanczosSinc(xii - x, 6.0f);
		rr = gg = bb = aa = 0;
			...
			IsIndexed())
		{
			...
			{
				auto pxptr = static_cast<uint8_t*>(BlindGetPixelPointer(xi - 5, yii));
				//calculate pointer to first byte in row
				kernelyc = KernelLanczosSinc(yii - y, 6.0f);
#if CXIMAGE_SUPPORT_ALPHA
				if (AlphaIsValid())
				{
 ...
 */
void CxImage::AddAveragingCont(const RGBQUAD& color, const float surf, float& rr, float& gg, float& bb, float& aa)
{
...
RGBQUAD CxImage::GetAreaColorInterpolated(
	const float xc, const float yc, const float w, const float h,
	const InterpolationMethod inMethod,
	const OverflowMethod ofMethod,
	RGBQUAD* const rplColor)
{
	...
	if (h > 1 && w > 1)
	{
		...
		intBL = area.CrossSection(CxRect2(static_cast<float>(xi1) - 0.5f, static_cast<float>(yi1) - 0.5f,
		                                  static_cast<float>(xi1) + 0.5f, static_cast<float>(yi1) + 0.5f));
		intTR = area.CrossSection(CxRect2(static_cast<float>(xi2) - 0.5f, static_cast<float>(yi2) - 0.5f,
		                                  static_cast<float>(xi2) + 0.5f, static_cast<float>(yi2) + 0.5f));
	...
	else
	{
		...
		for (y = yi1; y <= yi2; y++)
		{
			for (x = xi1; x <= xi2; x++)
			{
				intersect = area.CrossSection(CxRect2(static_cast<float>(x) - 0.5f, static_cast<float>(y) - 0.5f,
...
{
	if (fabs(x) > 1) return 0.0f;
	return (0.5f + 0.5f * static_cast<float>(cos(PI * x))) * (static_cast<float>(sin(PI * x)) / (PI * x));
}
...
float CxImage::KernelBessel_Order1(float x)
{
	...
	q = static_cast<float>(sqrt(2.0f / (PI * x))) * static_cast<float>(KernelBessel_P1(x) * (1.0f / sqrt(2.0f) * (sin(x)
