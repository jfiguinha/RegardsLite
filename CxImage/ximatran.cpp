	...
	case 24:
		{
			...
			if (info.nBkgndIndex >= 0) //translate transparency
				tmp.info.nBkgndColor = GetPaletteColor(static_cast<uint8_t>(info.nBkgndIndex));
...
bool CxImage::GrayScale()
{
	...
	else
	{
		...
		if (GetTransIndex() >= 0)
		{
			...
			ima.SetTransIndex(static_cast<uint8_t>(RGB2GRAY(c.rgbRed, c.rgbGreen, c.rgbBlue)));
		}
#if CXIMAGE_SUPPORT_SELECTION
		ima.SelectionCopy(*this);
#endif //CXIMAGE_SUPPORT_SELECTION
		...
		for (int32_t y = 0; y < head.biHeight; y++)
		{
			...
			{
				img[x8 + y * l8] = static_cast<uint8_t>(RGB2GRAY(*(iSrc+x+2), *(iSrc+x+1), *(iSrc+x+0)));
			}
...
bool CxImage::Flip(bool bFlipSelection, bool bFlipAlpha)
{
	...
	auto buff = static_cast<uint8_t*>(malloc(info.dwEffWidth));
...
bool CxImage::Mirror(bool bMirrorSelection, bool bMirrorAlpha)
{
	...
	auto imatmp = new CxImage(*this, false, true, true);
		...
		for (xs = 0; xs < newWidth; xs += RBLOCK)
		{
			...
			for (ys = 0; ys < newHeight; ys += RBLOCK)
			{
				...
				else
				{
					...
					for (x = xs; x < min(newWidth, xs+RBLOCK); x++)
					{
						info.nProgress = 100 * x / newWidth; //<Anatoly Ivasyuk>
						x2 = newWidth - x - 1;
						for (y = ys; y < min(newHeight, ys+RBLOCK); y++)
						{
		...
		for (xs = 0; xs < newWidth; xs += RBLOCK)
		{
			for (ys = 0; ys < newHeight; ys += RBLOCK)
			{
				if (head.biBitCount == 24)
				{
					...
					for (y = ys; y < min(newHeight, ys+RBLOCK); y++)
					{
						info.nProgress = 100 * y / newHeight; //<Anatoly Ivasyuk>
						y2 = newHeight - y - 1;
						dstPtr = static_cast<uint8_t*>(imgDest.BlindGetPixelPointer(xs, y));
						srcPtr = static_cast<uint8_t*>(BlindGetPixelPointer(y2, xs));
						for (x = xs; x < min(newWidth, xs+RBLOCK); x++)
						{
...
bool CxImage::Rotate(float angle, CxImage* iDst)
{
	...
	double ang = -angle * acos(static_cast<float>(0)) / 90;
	...
	newWidth = static_cast<int32_t>(floor(0.5f + rightTop.x - leftTop.x));
	newHeight = static_cast<int32_t>(floor(0.5f + leftBottom.y - leftTop.y));
	CxImage imgDest;
	imgDest.CopyInfo(*this);
	imgDest.Create(newWidth, newHeight, GetBpp(), GetType());
	imgDest.SetPalette(GetPalette());
	...
	{
		//RGB
		for (y = static_cast<int32_t>(leftTop.y), newY = 0; y <= static_cast<int32_t>(leftBottom.y); y++, newY++)
		{
			info.nProgress = 100 * newY / newHeight;
			if (info.nEscape) break;
			for (x = static_cast<int32_t>(leftTop.x), newX = 0; x <= static_cast<int32_t>(rightTop.x); x++, newX++)
			{
...
bool CxImage::Rotate2(float angle,
                      ...
                      RGBQUAD* replColor,
                      const bool optimizeRightAngles,
                      const bool bKeepOriginalSize)
{
	...
	//convert angle to radians and invert (positive angle performs clockwise rotation)
	float cos_angle = static_cast<float>(cos(ang)); //these two are needed later (to rotate)
	float sin_angle = static_cast<float>(sin(ang));
	...
	float minx = min(min(newp[0].x,newp[1].x), min(newp[2].x,newp[3].x));
	float miny = min(min(newp[0].y,newp[1].y), min(newp[2].y,newp[3].y));
	float maxx = max(max(newp[0].x,newp[1].x), max(newp[2].x,newp[3].x));
	float maxy = max(max(newp[0].y,newp[1].y), max(newp[2].y,newp[3].y));
	...
	float ssy = ((maxy + miny) - (static_cast<float>(newHeight) - 1)) / 2.0f; //start for y
	...
	if (!IsIndexed())
	{
		...
		uint8_t* pxptr;
#if CXIMAGE_SUPPORT_ALPHA
		uint8_t* pxptra = nullptr;
#endif //CXIMAGE_SUPPORT_ALPHA
		for (desty = 0; desty < newHeight; desty++)
		{
			info.nProgress = 100 * desty / newHeight;
			if (info.nEscape) break;
			//initialize x
			x = ssx;
			//calculate pointer to first byte in row
			pxptr = static_cast<uint8_t*>(imgDest.BlindGetPixelPointer(0, desty));
#if CXIMAGE_SUPPORT_ALPHA
			//calculate pointer to first byte in row
...
bool CxImage::Rotate180(CxImage* iDst)
{
	...
	for (y = 0; y < ht; y++)
	{
		info.nProgress = 100 * y / ht; //<Anatoly Ivasyuk>
		y2 = ht - y - 1;
		for (x = 0; x < wid; x++)
		{
...
bool CxImage::Resample(int32_t newx, int32_t newy, int32_t mode, CxImage* iDst)
{
	...
	xScale = static_cast<float>(head.biWidth) / static_cast<float>(newx);
	yScale = static_cast<float>(head.biHeight) / static_cast<float>(newy);
	...
	case 1: // nearest pixel
		{
			...
			{
				info.nProgress = 100 * y / newy;
				if (info.nEscape) break;
				fY = y * yScale;
				for (int32_t x = 0; x < newx; x++)
				{
					fX = x * xScale;
					newImage.SetPixelColor(x, y, GetPixelColor(static_cast<int32_t>(fX), static_cast<int32_t>(fY)));
				}
			}
			break;
		}
	case 2: // bicubic interpolation by Blake L. Carlson <blake-carlson(at)uiowa(dot)edu
		{
			...
			for (int32_t y = 0; y < newy; y++)
			{
				info.nProgress = 100 * y / newy;
				if (info.nEscape) break;
				f_y = static_cast<float>(y) * yScale - 0.5f;
				i_y = static_cast<int32_t>(floor(f_y));
				a = f_y - static_cast<float>(floor(f_y));
				for (int32_t x = 0; x < newx; x++)
				{
					f_x = static_cast<float>(x) * xScale - 0.5f;
					i_x = static_cast<int32_t>(floor(f_x));
					b = f_x - static_cast<float>(floor(f_x));

					rr = gg = bb = 0.0f;
					for (int32_t m = -1; m < 3; m++)
					{
						r1 = KernelBSpline(static_cast<float>(m) - a);
						yy = i_y + m;
						if (yy < 0) yy = 0;
						if (yy >= head.biHeight) yy = head.biHeight - 1;
						for (int32_t n = -1; n < 3; n++)
						{
							r2 = r1 * KernelBSpline(b - static_cast<float>(n));
							xx = i_x + n;
							if (xx < 0) xx = 0;
					...
					else
					{
						...
						*iDst++ = static_cast<uint8_t>(bb);
						*iDst++ = static_cast<uint8_t>(gg);
						*iDst = static_cast<uint8_t>(rr);
					}
				}
			}
			break;
	...
	default: // bilinear interpolation
		if (!(head.biWidth > newx && head.biHeight > newy && head.biBitCount == 24))
		{
			...
			for (int32_t y = 0; y < newy; y++)
			{
				...
				if (info.nEscape) break;
				fY = y * yScale;
				ifY = static_cast<int32_t>(fY);
				ifY1 = min(ymax, ifY+1);
				dy = fY - ifY;
				for (int32_t x = 0; x < newx; x++)
				{
					fX = x * xScale;
					ifX = static_cast<int32_t>(fX);
					ifX1 = min(xmax, ifX+1);
					dx = fX - ifX;
		...
		else
		{
			...
			auto naAccu = new int32_t[3 * newx + 3];
			auto naCarry = new int32_t[3 * newx + 3];
			int32_t* naTemp;
			...
			for (y = 0; y < head.biHeight; y++)
			{
				...
				else
				{
					...
					if (u < newx)
					{
						...
						for (j = 0; j < 3; j++) *pDest++ = static_cast<uint8_t>(naAccu[i++] / nScale);
					}
					naTemp = naCarry;
					naCarry = naAccu;
					naAccu = naTemp;
			...
			{
				// possibly not completed due to rounding errors
				for (i = 0; i < 3 * newx; i++) *pDest++ = static_cast<uint8_t>(naAccu[i] / nScale);
			}
			delete [] naAccu;
			delete [] naCarry;
		}
	...
	if (AlphaIsValid())
	{
		if (1 == mode)
		{
			...
			for (int32_t y = 0; y < newy; y++)
			{
				...
				{
					fX = x * xScale;
					newImage.AlphaSet(x, y, AlphaGet(static_cast<int32_t>(fX), static_cast<int32_t>(fY)));
				}
			}
		}
		else
...
bool CxImage::Resample2(
	...
	const InterpolationMethod inMethod,
	const OverflowMethod ofMethod,
	CxImage* const iDst,
	const bool disableAveraging)
{
	...
	uint8_t* pxptra = nullptr; // destination alpha data
#endif
	...
	if ((xScale <= 1 && yScale <= 1) || disableAveraging)
	{
		...
		if (!IsIndexed())
		{
			...
			for (dY = 0; dY < newy; dY++)
			{
				info.nProgress = 100 * dY / newy;
				if (info.nEscape) break;
				sY = (dY + 0.5f) * yScale - 0.5f;
				pxptr = static_cast<uint8_t*>(newImage.BlindGetPixelPointer(0, dY));
#if CXIMAGE_SUPPORT_ALPHA
				...
				for (dX = 0; dX < newx; dX++)
				{
					...
					q = GetPixelColorInterpolated(sX, sY, inMethod, ofMethod, nullptr);
					*pxptr++ = q.rgbBlue;
					*pxptr++ = q.rgbGreen;
					*pxptr++ = q.rgbRed;
#if CXIMAGE_SUPPORT_ALPHA
		...
		else
		{
			...
			for (dY = 0; dY < newy; dY++)
			{
				...
				{
					sX = (dX + 0.5f) * xScale - 0.5f;
					newImage.SetPixelColor(dX, dY, GetPixelColorInterpolated(sX, sY, inMethod, ofMethod, nullptr),
					                       true);
				} //for x
			} //for y
		} //if
		...
		for (dY = 0; dY < newy; dY++)
		{
			...
			for (dX = 0; dX < newx; dX++)
			{
				...
				newImage.SetPixelColor(
					dX, dY, GetAreaColorInterpolated(sX, sY, xScale, yScale, inMethod, ofMethod, nullptr), true);
			} //for x
		} //for y
	} //if

#if CXIMAGE_SUPPORT_ALPHA
	if (AlphaIsValid() && pxptra == nullptr)
	{
		for (int32_t y = 0; y < newy; y++)
		{
...
bool CxImage::DecreaseBpp(uint32_t nbit, bool errordiffusion, RGBQUAD* ppal, uint32_t clrimportant)
{
	...
	CxImage tmp;
	tmp.CopyInfo(*this);
	tmp.Create(head.biWidth, head.biHeight, static_cast<uint16_t>(nbit), info.dwType);
	...
	for (int32_t y = 0; y < head.biHeight; y++)
	{
		...
		for (int32_t x = 0; x < head.biWidth; x++)
		{
			...
			else
			{
				...
				er = static_cast<int32_t>(c.rgbRed) - static_cast<int32_t>(ce.rgbRed);
				eg = static_cast<int32_t>(c.rgbGreen) - static_cast<int32_t>(ce.rgbGreen);
				eb = static_cast<int32_t>(c.rgbBlue) - static_cast<int32_t>(ce.rgbBlue);

				c = GetPixelColor(x + 1, y);
				c.rgbRed = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbRed + ((er*7)/16))));
				c.rgbGreen = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbGreen + ((eg*7)/16))));
				c.rgbBlue = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbBlue + ((eb*7)/16))));
				...
				for (int32_t i = -1; i < 2; i++)
				{
					...
					c.rgbRed = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbRed + ((er * coeff)/16))));
					c.rgbGreen = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbGreen + ((eg * coeff)/16))));
					c.rgbBlue = min((uint8_t)255L, max((uint8_t)0L,(uint8_t)(c.rgbBlue + ((eb * coeff)/16))));
...
#define dth_MaxDitherIntensityVal (dth_NumRows*dth_NumCols*(dth_NumIntensityLevels-1))
			...
			int32_t DitherMatrix[dth_NumRows][dth_NumCols] = {
	...
	case 2:
		{
			...
			for (int32_t y = 0; y < head.biHeight; y++)
			{
				...
				for (int32_t x = 0; x < head.biWidth; x++)
				{
					...
					level = static_cast<uint8_t>(min(255, max(0, (int32_t)nlevel)));
					...
					for (i = -2; i < 3; i++)
					{
						...
						level = min((uint8_t)255, max((uint8_t)0, (uint8_t)nlevel));
	...
	case 7:
		{
			...
			auto Bmatrix = static_cast<uint8_t*>(malloc(size * sizeof(uint8_t)));
	...
	case 9:
		{
			...
			const int32_t pattern16x16[16][16] = {
 ...
 * \author  [VATI]
 */
bool CxImage::CropRotatedRectangle(int32_t topx, int32_t topy, int32_t width, int32_t height, float angle,
                                   CxImage* iDst)
{
	if (!pDib) return false;
	...
	if (false == tmp.Rotate(static_cast<float>(-angle * 57.295779513082320877))) // Rotate expects angle in degrees
		return false;
	...
	case 4:
		{
			...
			{
				info.nProgress = 100 * (y - starty) / (endy - starty); //<Anatoly Ivasyuk>
...
bool CxImage::Skew(float xgain, float ygain, int32_t xpivot, int32_t ypivot, bool bEnableInterpolation)
{
	...
	{
		info.nProgress = 100 * (y - ymin) / (ymax - ymin);
		if (info.nEscape) break;
		for (int32_t x = xmin; x < xmax; x++)
		{
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				{
					tmp.SetPixelColor(x, y, GetPixelColorInterpolated(nx, ny, IM_BILINEAR, OM_BACKGROUND), true);
				}
				else
#endif //CXIMAGE_SUPPORT_INTERPOLATION
				{
					if (head.biClrUsed == 0)
					{
						tmp.SetPixelColor(x, y, GetPixelColor(static_cast<int32_t>(nx), static_cast<int32_t>(ny)));
					}
					else
					{
						tmp.SetPixelIndex(x, y, GetPixelIndex(static_cast<int32_t>(nx), static_cast<int32_t>(ny)));
					}
#if CXIMAGE_SUPPORT_ALPHA
					tmp.AlphaSet(x, y, AlphaGet(static_cast<int32_t>(nx), static_cast<int32_t>(ny)));
#endif //CXIMAGE_SUPPORT_ALPHA
				}
			}
		}
	...
	case 4:
		{
			...
			{
				info.nProgress = 100 * y / newHeight;
	...
	case 24:
		{
			...
			{
				info.nProgress = 100 * y / (1 + top - bottom);
...
bool CxImage::Expand(int32_t newx, int32_t newy, RGBQUAD canvascolor, CxImage* iDst)
{
	...
	return Expand(nAddLeft, nAddTop, newx - (head.biWidth + nAddLeft), newy - (head.biHeight + nAddTop), canvascolor,
	              iDst);
}
...
bool CxImage::Thumbnail(int32_t newx, int32_t newy, RGBQUAD canvascolor, CxImage* iDst)
{
	...
	{
		float fScale;
		float fAspect = static_cast<float>(newx) / static_cast<float>(newy);
		if (fAspect * head.biHeight > head.biWidth)
		{
			fScale = static_cast<float>(newy) / head.biHeight;
		}
		else
		{
			fScale = static_cast<float>(newx) / head.biWidth;
		}
		tmp.Resample(static_cast<int32_t>(fScale * head.biWidth), static_cast<int32_t>(fScale * head.biHeight), 0);
	}
		...
		for (int32_t x = xmin; x < xmax; x++)
		{
...
#endif //CXIMAGE_SUPPORT_SELECTION
			{
				...
				if (radius < rmax)
				{
					...
					{
						nx = static_cast<int32_t>(fabs((angle * xmax / 6.2831852)));
						ny = static_cast<int32_t>(fabs((radius * ymax / rmax)));
					}
					else
					{
						nx = x + (x % 32) - 16;
...
bool CxImage::QIShrink(int32_t newx, int32_t newy, CxImage* const iDst, bool bChangeBpp)
{
	...
	auto accu = new uint32_t[newx * accuCellSize]; //array for suming pixels... one pixel for every destination column
	...
	if (!IsIndexed())
	{
		...
		srcPtrS = static_cast<uint8_t*>(BlindGetPixelPointer(0, 0));
		destPtrS = static_cast<uint8_t*>(newImage.BlindGetPixelPointer(0, 0));
		...
		for (int32_t y = 0; y < oldy; y++)
		{
			//for all source rows
			info.nProgress = 100 * y / oldy;
			if (info.nEscape) break;
			ey += newy;
...
#if CXIMAGE_SUPPORT_ALPHA
					if (alphaPtr) *(alphaPtr++) = static_cast<uint8_t>(*(accuPtr + 4) / *(accuPtr + 3));
#endif
					accuPtr += accuCellSize;
				} //for k
				memset(accu, 0, newx * accuCellSize * sizeof(uint32_t)); //clear accu
...
#if CXIMAGE_SUPPORT_ALPHA
					if (pAlpha) rgb.rgbReserved = static_cast<uint8_t>(*(accuPtr + 4) / *(accuPtr + 3));
#endif
					newImage.SetPixelColor(dx, dy, rgb, pAlpha != nullptr);
					accuPtr += accuCellSize;
				} //for dx
				memset(accu, 0, newx * accuCellSize * sizeof(uint32_t)); //clear accu
				dy++;
