...
bool CxImageJBG::Decode(CxFile* hFile)
{
	...
	size_t len, cnt;
	unsigned char *buffer = nullptr, *p;
	int32_t result;
	...
	cx_try
	{
		...
		buffer = static_cast<uint8_t*>(malloc(JBIG_BUFSIZE));
		if (!buffer)
			cx_throw("Sorry, not enough memory available!");
...
bool CxImageJBG::Encode(CxFile* hFile)
{
	...
	auto buffer = static_cast<unsigned char*>(malloc(ew * h * 2));
	if (!buffer)
	{
