...
#endif
	wxASSERT_MSG((paths.GetCount() == names.GetCount()),
	             wxT("The number of paths and their human readable names should be equal in number."));
...
int setdrive(int drive)
{
	...
	if (drive < 1 || drive > 31)
		return -1;
	newdrive[0] = static_cast<wxChar>((wxT('A') + drive - 1));
	newdrive[1] = wxT(':');
	newdrive[2] = wxT('\0');
#if defined(__WINDOWS__)
...
bool wxIsDriveAvailable(const wxString& dirName)
{
	...
	if (dirName.length() == 3 && dirName[static_cast<size_t>(1)] == wxT(':'))
	{
                              ...
                              const wxString& name)
{
	...
	m_showHidden = false;
	auto rootData = new wxDirItemData(wxEmptyString, wxEmptyString, true);
...
wxCheckTree* wxGenericDirCtrl::CreateTreeCtrl(wxWindow* parent, wxWindowID treeid, const wxPoint& pos,
                                              const wxSize& size, long treeStyle)
{
...
{
	auto dir_item = new wxDirItemData(path, name, true);
...
void wxGenericDirCtrl::PopulateNode(wxTreeItemId parentId)
{
	...
	for (i = 0; i < dirs.GetCount(); i++)
	{
		...
		auto dir_item = new wxDirItemData(path, eachFilename, true);
	...
	if (!HasFlag(wxDIRCTRL_DIR_ONLY))
	{
		for (i = 0; i < filenames.GetCount(); i++)
		{
			...
			auto dir_item = new wxDirItemData(path, eachFilename, false);
...
wxString wxGenericDirCtrl::GetFilePath() const
{
	...
	if (treeid)
	{
		auto data = static_cast<wxDirItemData*>(m_treeCtrl->GetItemData(treeid));
		if (data->m_isDir)
			return wxEmptyString;
		return data->m_path;
	}
                                 ...
                                 long style)
{
	...
	{
		style |= parent->HasFlag(wxDIRCTRL_3D_INTERNAL)
			         ? wxBORDER_SUNKEN
			         : wxBORDER_NONE;
	}
#ifdef __WXGTK__
...
void wxDirFilterListCtrl::FillFilterList(const wxString& filter, int defaultFilter)
{
	...
	size_t n = static_cast<size_t>(wxParseCommonDialogsFilter(filter, descriptions, filters));

	if (n > 0 && defaultFilter < static_cast<int>(n))
	{
...
static wxBitmap CreateAntialiasedBitmap(const wxImage& img, const wxSize& sz)
{
	...
	for (y = 0; y < size; y++)
	{
		for (x = 0; x < size; x++)
		{
			...
			{
				ps[0] = static_cast<unsigned char>(sr >> 2);
				ps[1] = static_cast<unsigned char>(sg >> 2);
				ps[2] = static_cast<unsigned char>(sb >> 2);
			}
			ps += 3;
		}
...
int wxFileIconsTable::GetIconID(const wxString& extension, const wxString& mime)
{
	...
	if (!extension.empty())
	{
		auto entry = static_cast<wxFileIconEntry*>(m_HashTable->Get(extension));
